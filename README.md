...
命令编号属于功能性事实，不可版权保护，我据此用 C++/QML 独立实现；下面是知识来源，
按对项目的贡献排序：
...
> 说明：本项目依据的是**协议事实**（帧格式、命令编号、字节布局），这些属于功能性规范，
> 不受版权保护；代码为独立实现。上述资料中部分是 GPL 授权的*文档*，本项目未复制其代码，
> 因此采用 MIT 授权。如果你的使用场景需要更保守的处理，请自行评估。
